...
func TestSignAndVerify(t *testing.T) {
	...
	for _, test := range tests {
		...
		gotSig, success := sign(&privKey.Key, nonce, hash)
		if !success {
		...
		gotCode, err := gotSig.RecoveryCode()
		if err != nil {
			t.Errorf("%s: unexpected error getting recovery code: %v", test.name, err)
			continue
		}
		if gotCode != test.wantCode {
			t.Errorf("%s: unexpected recovery code -- got %x, want %x",
				test.name, gotCode, test.wantCode)
			continue
		}
		...
		// Attempt to re-generate recovery code, make sure it is still the same
		if !gotSig.BruteforceRecoveryCode(hash, pubKey) {
			t.Errorf("%s: failed to compute recovery code", test.name)
		}

		gotCode2, err := gotSig.RecoveryCode()
		if err != nil {
			t.Errorf("%s: unexpected error getting computed recovery code: %v", test.name, err)
		} else if gotCode2 != test.wantCode {
			t.Errorf("%s: unexpected computed recovery code -- got %x, want %x",
				test.name, gotCode2, test.wantCode)
		}
...
func TestSignFailures(t *testing.T) {
	...
	for _, test := range tests {
		...
		sig, success := sign(privKey, nonce, hash)
		if success {
...
// TestRecoveryCodeNoRecoveryCode ensures RecoveryCode returns an error when the
// signature was created without a recovery code.
func TestRecoveryCodeNoRecoveryCode(t *testing.T) {
	var r, s ModNScalar
	r.SetInt(1)
	s.SetInt(2)
	sig := NewSignature(&r, &s)

	_, err := sig.RecoveryCode()
	if err == nil {
		t.Fatal("expected error from RecoveryCode on signature without recovery code")
	}
	if !errors.Is(err, ErrSigInvalidRecoveryCode) {
		t.Fatalf("expected ErrSigInvalidRecoveryCode, got %v", err)
	}
}

// TestRecoverPublicKeyNoRecoveryCode ensures RecoverPublicKey returns an error
// when the signature was created without a recovery code.
func TestRecoverPublicKeyNoRecoveryCode(t *testing.T) {
	var r, s ModNScalar
	r.SetInt(1)
	s.SetInt(2)
	sig := NewSignature(&r, &s)
	...
	hash := make([]byte, 32)
	_, err := sig.RecoverPublicKey(hash)
	if err == nil {
		t.Fatal("expected error from RecoverPublicKey on signature without recovery code")
	}
	if !errors.Is(err, ErrSigInvalidRecoveryCode) {
		t.Fatalf("expected ErrSigInvalidRecoveryCode, got %v", err)
	}
...
// TestNewSignatureWithRecoveryCodeValidation ensures that invalid recovery codes
// are rejected.
func TestNewSignatureWithRecoveryCodeValidation(t *testing.T) {
	var r, s ModNScalar
	r.SetInt(1)
	s.SetInt(2)

	// Valid recovery codes (0-3) should not panic.
	for v := byte(0); v <= 3; v++ {
		func() {
			defer func() {
				if r := recover(); r != nil {
					t.Errorf("unexpected panic for valid recovery code %d: %v", v, r)
				}
			}()
			NewSignatureWithRecoveryCode(&r, &s, v)
		}()
	}

	// Invalid recovery code (4) should panic.
	func() {
		defer func() {
			if r := recover(); r == nil {
				t.Error("expected panic for invalid recovery code 4")
			}
		}()
		NewSignatureWithRecoveryCode(&r, &s, 4)
	}()
}
